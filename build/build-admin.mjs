...
// SHA-256 of the admin passphrase. The plaintext is NEVER stored here; only the
// hash. Change by computing:
//   node -e "console.log(require('crypto').createHash('sha256').update('YOUR_PASSPHRASE').digest('hex'))"
...
(async function () {
  ...
  // Prompt once. We only reach here if the user arrived via the 5-tap gesture
  // (or guessed the URL). Either way, a wrong answer redirects home silently.
...
// Reveal body only once unlocked.
